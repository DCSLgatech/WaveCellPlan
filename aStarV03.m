...
Description: A* algorithm; Handles mmultiple goals when any one goal will
do 
%}

function nodeData = aStarV03(G, nodeS, nodeG, heur)
...
%	noeData(n).d	= cost to come
%	nodeDat(n).b	= backpointer
...
while (nOpen ~= 0) && (~goalCl)
	...
	for k = 1:numel(nodeG)
		if nodeData(nodeG(k)).mk == 2, goalCl = 1; break; end
	end
end
